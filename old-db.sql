...
-- Fix existing username inconsistencies first
UPDATE pharmacy_users SET username = 'admin' WHERE username = 'admin.pharmacy' AND role = 'admin';
DELETE FROM pharmacy_users WHERE username = 'alice.brown.mgr' AND role = 'manager';
...
-- Admin user (handle existing admin.pharmacy user)
DO $$
BEGIN
...
EXCEPTION WHEN unique_violation OR check_violation THEN
    ...
    UPDATE pharmacy_users SET 
        ...
        is_active = true
    WHERE (username = 'admin' OR username = 'admin.pharmacy') AND role = 'admin';
END $$;

-- Manager user (ensure only one manager exists)
DO $$
BEGIN
    -- First check if manager exists and clean up duplicates
    IF EXISTS (SELECT 1 FROM pharmacy_users WHERE role = 'manager') THEN
        -- Keep only one manager and update it
        DELETE FROM pharmacy_users WHERE role = 'manager' AND username != 'manager';
        UPDATE pharmacy_users SET 
            username = 'manager',
            employee_id = 'EMP002',
            first_name = 'Manager',
        ...
        WHERE role = 'manager';
    ELSE
        -- Insert new manager if none exists
...
-- Salesperson user (force creation - this was missing)
-- First, let's see what might be blocking salesperson creation
DO $$
DECLARE
    existing_salesperson_count INTEGER;
    existing_emp003_count INTEGER;
BEGIN
    -- Check what exists
    SELECT COUNT(*) INTO existing_salesperson_count FROM pharmacy_users WHERE username = 'salesperson';
    SELECT COUNT(*) INTO existing_emp003_count FROM pharmacy_users WHERE employee_id = 'EMP003';
    
    RAISE NOTICE 'Existing salesperson username count: %', existing_salesperson_count;
    RAISE NOTICE 'Existing EMP003 employee_id count: %', existing_emp003_count;
    
    -- Delete any conflicting records first
    DELETE FROM pharmacy_users WHERE username = 'salesperson' OR employee_id = 'EMP003';
    
    -- Now force insert salesperson
    INSERT INTO pharmacy_users (username, employee_id, first_name, last_name, role, phone, is_active) 
    VALUES ('salesperson', 'EMP003', 'Sales', 'Person', 'salesperson', '+234-800-123-4568', true);
    
    RAISE NOTICE 'Salesperson created successfully!';
    
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error creating salesperson: % %', SQLSTATE, SQLERRM;
    -- Try alternative approach if there's still a conflict
    UPDATE pharmacy_users SET 
        ...
        is_active = true
    WHERE role = 'salesperson' OR username = 'salesperson' OR employee_id = 'EMP003';
    
    -- If update didn't work, force insert with different employee_id
    IF NOT FOUND THEN
        INSERT INTO pharmacy_users (username, employee_id, first_name, last_name, role, phone, is_active) 
        VALUES ('salesperson', 'EMP003X', 'Sales', 'Person', 'salesperson', '+234-800-123-4568', true);
        RAISE NOTICE 'Salesperson created with alternative employee_id: EMP003X';
    END IF;
END $$;

-- Final cleanup: Ensure exactly 3 users
DELETE FROM pharmacy_users WHERE id NOT IN (
...
-- Insert sample sales (only if salesperson exists)
...
FROM (SELECT id FROM pharmacy_users WHERE role = 'salesperson' LIMIT 1) salesperson_user,
     generate_series(1, 5)
WHERE salesperson_user.id IS NOT NULL;
...
-- Debug: Check if salesperson was created
SELECT 
    'FINAL USER COUNT' as check_type,
...
-- Final salesperson check
SELECT 
    'SALESPERSON CHECK' as check_type,
    CASE 
        WHEN EXISTS (SELECT 1 FROM pharmacy_users WHERE role = 'salesperson') 
        THEN '✅ Salesperson exists' 
        ELSE '❌ Salesperson still missing'
    END as status
FROM pharmacy_users
LIMIT 1;
